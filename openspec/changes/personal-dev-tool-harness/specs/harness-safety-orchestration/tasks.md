...
- [ ] `SafetyHookName` 包含 5 个枚举值
- [ ] `SafetyEvent` 包含 `schemaVersion: "safety-event.v1"`
- [ ] `npx tsc --noEmit` 通过
...
#### 验收标准
- [ ] 正确规范化 bash 命令
- [ ] 正确规范化 PowerShell 命令（`Remove-Item -Recurse -Force`）
- [ ] 正确规范化 cmd 命令
- [ ] 多余空白被去除
- [ ] `npx tsc --noEmit` 通过
...
- [ ] Hook 路由正确分派
- [ ] 未知 Hook 返回 2805
- [ ] push 门禁正确检查 review 状态
- [ ] session-summary 写入事件
- [ ] 对应测试绿灯
...
- [ ] `harness doctor --json` 输出安全配置摘要
- [ ] 危险命令被正确阻断
- [ ] 敏感文件被正确跳过
- [ ] 事件文件存在于 `.harness/events/`
- [ ] 事件文件不含敏感内容
